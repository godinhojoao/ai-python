...
- Examples with `numpy` and `matplotlib`: arrays, operations and linear algebra.
...
- `ndarray` = n-dimensional array.
  ...
  - `axis = index of dimension`, 2D array has axis 0: rows and 1: columns.
- `ufunc` = universal function, an operation applied element-wise (np.add, np.maximum, np.sin).
- `broadcasting` = how numpy treats arrays with different shapes in arithmetic ops: the smaller array is broadcast across the larger so their shapes become compatible (element-wise ops need matching shapes).
  - The stretching is only conceptual, numpy doesn't actually copy the data.
    ...
    print(a)
    print(a * 2) # 2. 4. 6.
    # [1. 2. 3.] * [2. 2. 2.] (broadcasting)
    ```

## More in the code examples at: [4-numpy.py](4-numpy.py)
